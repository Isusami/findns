...
### اسکن با تست واقعی MasterDnsVPN (اختیاری)

</div>

```bash
findns scan -i resolvers.txt -o results.json \
  --masterdns-domain v.example.com \
...
این مرحله برای هر resolver یک‌بار `masterdnsvpn-client` را اجرا می‌کند، یک
`client_config.toml` تک‌بار‌مصرف کنار آن می‌گذارد، و از طریق پروکسی محلی
SOCKS5 یک CONNECT واقعی به `example.com:80` می‌فرستد. resolver وقتی موفق
حساب می‌شود که اولین پاسخ SOCKS5 در زمان `--e2e-timeout` بازگردد.

پشتیبانی از چند دامنه با تکرار فلگ:
...
- نیازمند: `masterdnsvpn-client` در PATH یا کنار باینری findns. دانلود از: https://github.com/masterking32/MasterDnsVPN/releases/latest
- اگر `client_config.toml` کنار findns موجود باشد، خودکار به عنوان قالب پایه استفاده می‌شود و فقط کلیدهای ضروری بازنویسی می‌شوند.
- کلید رمزنگاری هرگز روی command line قرار نمی‌گیرد؛ فقط داخل فایل TOML با مجوز `0600` نوشته می‌شود.
- متریک: `mdvpn_e2e_ms`. با `--masterdns-mtu-bisect` متریک‌های `mdvpn_up_mtu` و `mdvpn_down_mtu` هم اضافه می‌شوند (کندتر).
- روش رمزنگاری باید با سرور یکی باشد (مثلاً `--masterdns-encryption-method 3` برای AES-128-GCM).
